...
- [x] OpenAI API로 공고/RFP에서 `사업 개요 / 요구사항 / 평가항목`을 추출하고 사용자가 확정한다
...
11) 현재 구현의 outline flat list / 로컬 citation 검색은 임시 구현이며, 목표 상태는 아니다.
...
14) RFP 검토 화면의 canonical schema는 `사업 개요 / 요구사항 / 평가항목`으로 둔다.
15) 공고 파일은 업로드 시 자동 추출하지 않고, 사용자가 `전체 파일로 추출`을 눌렀을 때만 OpenAI 호출을 수행한다.
16) RFP 추출 프롬프트는 섹션별로 분리하고, 수정 위치는 `api/app/services/rfp_prompts.py`로 고정한다.

## 4. 현재 상태 요약
- 상태: `.env` 기반 OpenAI 설정 로딩, 멀티파일 RFP 업로드/role 지정, section별 prompt 기반 `사업 개요 / 요구사항 / 평가항목` 추출, retrieval 기반 AI 초안 생성, draft chat/apply 편집 루프까지 연결됨. 초기 MVP는 완료
- 우선순위: 1) 추출 근거/evidence 가시화 2) 초안 생성 시 수동 source pinning 3) 매핑/서식 고도화

## 5. 다음 작업(Next Actions)
1) RFP 추출 결과별로 사용된 chunk 근거를 보여주고, 파일/근거를 수동 포함·제외할 수 있게 한다
2) 초안 생성 전에 요구사항/평가항목 기준으로 source pinning과 section별 생성 제어를 추가한다
3) 평가항목 매핑과 export 서식 고도화를 재개한다
